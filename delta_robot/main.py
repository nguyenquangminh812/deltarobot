...
def CalStepVel(startP,endP):
    ...
    for i in range(sample):
        calStepNum[i][0] = int((stepNum[i][0] - stepNum[i+1][0])/(360 / 800 / vibuoc) *Tiso); 
        calStepNum[i][1] = int((stepNum[i][1] - stepNum[i+1][1])/(360 / 800 / vibuoc) *Tiso); 
        calStepNum[i][2] = int((stepNum[i][2] - stepNum[i+1][2])/(360 / 800 / vibuoc) *Tiso);
    # print(calStepNum) 
